...
2. **필로소피 AI 툴** — 직접 만든 AI 학습 도구 (카테고리 탭으로 분류 표시)
   - 언어·독서
     ...
     - 아모배 영어 — https://amobeenglish.netlify.app/
     - JEV 학습 앱 — https://philojev.netlify.app/
   - 수학·물리
     ...
     - 3D 물리 놀이터 — https://philophysics3d.netlify.app/
     - 3D 물리학습 앱 (GPT-6 Astra 제작) — https://philogptphysics.netlify.app/
   - 과학·자연
   ...
   - 진로·AI·게임
     ...
     - 역방향 코드 학습기 — https://philocodeteacher.netlify.app/
   - 직장·HR
...
- **강연 후기** — 네이버 카페에 올라온 강연·연수 후기를 교육 신청 예정자에게 보여주기 위한 전용 카테고리. 검증된 후기만 노출되도록 **관리자(대표님)만 작성 가능**하도록 Firestore 규칙(`firestore.rules`)과 글쓰기 폼에서 함께 제한하며, 목록에서는 골드 톤으로 강조 표시됩니다. `education-inquiry.html`의 "강연 후기" 섹션(`#reviews`)에 최신 6개가 카드로 미리보기되고, "후기 더 보기" 버튼으로 `community.html?cat=강연 후기` 필터 화면으로 연결됩니다. 후기가 하나도 없으면 education-inquiry.html의 해당 섹션은 자동으로 숨겨집니다
  - 이 카테고리는 원 작성자(네이버 카페에 후기를 남긴 분)를 별도 입력란(후기 작성자 이름·소속)에 직접 입력해 `authorName`/`authorAffiliation`으로 저장합니다 — 다른 카테고리처럼 로그인한 관리자 본인 이름이 자동으로 들어가지 않습니다.
  - "전체 붙여넣기" 상자에 `제목: …` / `작성자: …` / `소속: …` 줄과 `---` 구분선, 그 아래 본문을 한 번에 붙여넣고 "자동 채우기"를 누르면 제목·작성자·소속·본문 입력란에 자동으로 나뉘어 채워집니다 (형식에 맞지 않으면 첫 줄을 제목으로, 나머지 전체를 본문으로 사용)
